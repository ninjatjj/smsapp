...
public class ConversationActivity extends AbstractConversationActivity
	...
	private ServiceConnection mConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			TextView findViewById = (TextView) findViewById(R.id.conversationWith);
			findViewById.setText(ContactManager.getDisplayName(
					getContentResolver(), address));

			TreeSet<Sms> treeSet = new TreeSet<Sms>(Collections.reverseOrder());
			treeSet.addAll(application.getClient().getMessages());
			for (Sms sms : treeSet) {
				messageReceived(sms);
			}
			application.getClient().addMessageListener(
	...
	protected void onDestroy() {
		super.onDestroy();
		if (application != null) {
			application.getClient().removeMessageListener(this);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.reconnect:
			application.getClient().reconnect();
			return true;
		case R.id.settings:
	...
	protected boolean connected() {
		return application.getClient().connected();
	}
